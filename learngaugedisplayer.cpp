...
LearnGaugeDisplayer::LearnGaugeDisplayer(QWidget *parent) :
        QWidget(parent)
{
    this->initVariables();
}

LearnGaugeDisplayer::~LearnGaugeDisplayer()
{
    delete updateTimer;
    delete singleTimer;
}

void LearnGaugeDisplayer::initVariables()
{
...
void LearnGaugeDisplayer::paintEvent(QPaintEvent *)
{
    QPainter painter;
    painter.begin(this);
...
void LearnGaugeDisplayer::drawOuterCircle(QPainter *painter)
{
    painter->save();
...
void LearnGaugeDisplayer::drawInnerCircle(QPainter *painter)
{
    painter->save();
...
void LearnGaugeDisplayer::drawMark(QPainter *painter)
{
    painter->save();
    ...
    for(int i=0;i<=100;i++)
    {
        ...
        if(i%10==0)
        {
            ...
            QPointF textPot(-textWidth/2,m_colorCircleRadius+SPACE1+LONG1+SPACE1+textHeight);
            painter->translate(textPot);
            painter->rotate(180);
            painter->setPen(Qt::black);
            qreal rectWidth=(m_innerRadius-m_coverCircleRadius)/4;
            qreal fontSize=rectWidth*1;
            QFont font;
...
void LearnGaugeDisplayer::drawCoverBall(QPainter *painter)
{
    painter->save();
...
void LearnGaugeDisplayer::drawTextRect(QPainter *painter)
{
    painter->save();
...
void LearnGaugeDisplayer::drawCoverCircle(QPainter *painter)
{
    painter->save();
    painter->setPen(Qt::NoPen);
...
void LearnGaugeDisplayer::drawColorPie(QPainter *painter)
{
    painter->save();
...
void LearnGaugeDisplayer::drawIndicator(QPainter *painter)
{
    painter->save();
    painter->translate(m_center);
    painter->setRenderHints(QPainter::Antialiasing|QPainter::TextAntialiasing);
...
void LearnGaugeDisplayer::resetVariables()
{
    m_outerRadius=width()>height() ? height()/2: width()/2;
    m_innerRadius=m_outerRadius/8*7;
    m_coverCircleRadius=m_outerRadius/8*4+10;
...
void LearnGaugeDisplayer::setValue(qreal value)
{
    if(value>m_value)
    {
        m_bReverse=false;
...
void LearnGaugeDisplayer::UpdateGraph()
{
    if(!m_bReverse)
    {
        m_currentValue+=0.5;
